...
const DataTable: React.FC = observer(() => {
  ...
  if (!tableContainerRef || !tableStore.currentTable || 
    tableStore.isLoading || tableStore.isLoadingMore || !tableStore.hasMore) return;
...
}, [tableContainerRef, tableStore.currentTable?.id, tableStore.isLoading, tableStore.isLoadingMore, tableStore.hasMore]);
...
// console.log('Columns:', columns);
  const validateField = (fieldId: string, value: string): string => {
    const column = tableStore.currentTable?.columns.find(c => c.id === fieldId);
    ...
    switch (column.type) {
      case 'number':
        ...
        break;
      case 'date':
        if (!/^\d{4}-\d{2}-\d{2}$/.test(value)) return 'Date format: YYYY-MM-DD';
        break;
      default:
    ...
    <Paper sx={{ p: 3, width: '100%' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
        ...
        <Button
          ...
          startIcon={tableStore.isLoading ? <CircularProgress size={20} /> : null}
        >
          Add Record
        </Button>
      </Box>
            ...
            <Stack spacing={3}>
                  ...
                  <FormControl fullWidth>
                    ...
                    <Select
                      ...
                      onChange={(e) => {
                        ...
                        setCurrentError(validateField(e.target.value as string, currentValue));
                      }}
                      label="Select Field"
                ...
                {selectedFields.length === 0 ? (
                  ...
                  <Stack spacing={1}>
                    {selectedFields.map((field, index) => {
                      const column = tableStore.currentTable?.columns.find(c => c.id === field.fieldId);
                      return (
                        <Paper 
                          key={index} 
                          variant="outlined" 
                          ...
                          <Stack direction="row" justifyContent="space-between" alignItems="center">
                            <Box>
                              <Typography fontWeight="medium">
                                {column?.label}: <Typography component="span">{field.value}</Typography>
                              </Typography>
                              {field.error && (
              ...
              {fieldsMissing > 0 && (
                <Alert severity="warning">
                  Need to add {fieldsMissing} more fields (minimum {minFieldsRequired})
                </Alert>
              )}
            </Stack>
          </DialogContent>
